...
export function parseColor(css) {
  ...
  // rgb() / rgba()
    ...
    if (m) {
      const r =
        parseFloat(m[1]) > 1 ? parseFloat(m[1]) / 255 : parseFloat(m[1]);
      const g =
        parseFloat(m[2]) > 1 ? parseFloat(m[2]) / 255 : parseFloat(m[2]);
      const b =
        parseFloat(m[3]) > 1 ? parseFloat(m[3]) / 255 : parseFloat(m[3]);
