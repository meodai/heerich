  ...
  for (let gx = 0; gx < gridSize; gx++) {
    for (let gz = 0; gz < gridSize; gz++) {
      ...
      const height =
        1 +
        ((gx + gz) % 4) +
        Math.round(Math.abs(Math.sin(gx * 0.7 + gz * 0.5)) * 4);
      ...
      h.addBox({
        ...
        style: {
          default: {
            fill: colors[ci],
            stroke: strokeColors[ci],
            strokeWidth: 0.4,
          },
          top: {
            fill: colors[(ci + 1) % colors.length],
            stroke: strokeColors[ci],
            strokeWidth: 0.4,
          },
        },
...
for (const size of [10, 20, 30]) {
  ...
  scenes[`perspective_${size}`] = makeGrid(size, {
    type: "perspective",
    distance: 80,
  });
}
...
// ---------------------------------------------------------------------------
// SVG render only (pre-computed faces)
// ---------------------------------------------------------------------------
boxplot(() => {
  summary(() => {
    bench("SVG render $size grid", function* (state) {
      const h = scenes[`oblique_${state.get("size")}`];
